...
rule count_mirbase_mature:
    input:
        bam=rules.umi_deduplication.output,
        bai=rules.index_deduplicated_bam.output
    output:
        counts=resolve_results_filepath(
            config.get("paths").get("results_dir"),"reads/counts/{sample}.mirna_mature.txt")
    conda:
...
rule count_mirbase_hairpin:
    ...
    output:
        counts=resolve_results_filepath(
            config.get("paths").get("results_dir"),"reads/counts/{sample}.mirna_hairpin.txt")
    conda:
        resolve_single_filepath(
            config.get("paths").get("workdir"),"workflow/envs/samtools.yaml")
    threads: conservative_cpu_count(reserve_cores=2, max_cores=99)
...
rule get_counts_all:
    ...
    output:
        touch(resolve_results_filepath(
            config.get("paths").get("results_dir"),"reads/counts/{sample}.counts.completed"))